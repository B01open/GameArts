...
static fs::path s_gameArtDir;
static fs::path s_gameArtIpcDir;

void dev::setDataDir(fs::path const& _dataDir)
{
    s_gameArtDir = _dataDir;
}

void dev::setIpcPath(fs::path const& _ipcDir)
{
    s_gameArtIpcDir = _ipcDir;
}

fs::path dev::getIpcPath()
{
    // Strip "geth.ipc" suffix if provided.
    if (s_gameArtIpcDir.filename() == "geth.ipc")
        return s_gameArtIpcDir.parent_path();
    else
        return s_gameArtIpcDir;
}

fs::path dev::getDataDir(string _prefix)
{
    if (_prefix.empty())
        _prefix = "game-art-data";
    if (_prefix == "game-art-data" && !s_gameArtDir.empty())
        return s_gameArtDir;
